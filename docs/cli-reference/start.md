...
Run OriginHub server or worker.
...
| `--system-default-container-registry` value | `docker.io`                            | Default container registry for OriginHub to pull system and inference images.<br/>Keep all OriginHub business images under the `gpustack` namespace; if a multi-level namespace is required, keep `gpustack` as the last level, e.g. `awesome.com/amazing-group/amazing-subgroup/gpustack`. Then set this flag (or the server/worker `system_default_container_registry` config key, or the `GPUSTACK_SYSTEM_DEFAULT_CONTAINER_REGISTRY` environment variable) to the parent path, e.g. `awesome.com/amazing-group/amazing-subgroup`.<br/>If the images cannot stay under the `gpustack` namespace (e.g. `gpustack-ai`), adjust `--image-repo` and `--benchmark-image-repo` accordingly. |
| `--image-repo` value                        | `gpustack/gpustack`                    | Override the default image repository for the OriginHub container. Do not include the tag in the value; OriginHub appends the version tag automatically.<br/>If the image `gpustack/gpustack:<tag>` lives outside the `gpustack` namespace, e.g. `awesome.com/amazing-group/amazing-subgroup/gpustack-ai/gpustack:<tag>`, first point `--system-default-container-registry=awesome.com/amazing-group/amazing-subgroup` to the registry with the parent namespace, then set this flag (or the server/worker `image_repo` config key, or the `GPUSTACK_IMAGE_REPO` environment variable) to `gpustack-ai/gpustack`. |
...
| `--namespace` value                         | (empty)                                | Kubernetes namespace for OriginHub to deploy gateway routing rules and model instances.                                                |
...
| `--api-port` value                               | `30080`                                          | Port to bind the OriginHub API server to.                                                                                                                                                                                                                                                                                                      |
| `--proxy-port` value                             | `30079`                                          | Port to bind the OriginHub proxy server to.                                                                                                                                                                                                                                                                                                    |
...
| `--disable-worker`                               | (empty)                                          | (DEPRECATED) Disable the embedded worker for the OriginHub server. New installations will not have the embedded worker by default. Use '--enable-worker' to enable the embedded worker if needed. If neither flag is set, for backward compatibility, the embedded worker will be enabled by default for legacy installations prior to v2.0.1. |
| `--enable-worker`                                | `False`                                          | Enable the embedded worker for the OriginHub server.                                                                                                                                                                                                                                                                                           |
...
| `--ssl-ca-certfile` value                        | (empty)                                          | Path to the PEM CA bundle workers use to verify the OriginHub server. Required when the server certificate is signed by a private CA.                                                                                                                                                                                                      |
...
| `--cas-username-attribute` value                 | (empty)                                          | CAS XML attribute name to use as the OriginHub username. Defaults to the CAS `cas:user` element when unset.                                                                                                                                                                                                                                    |
...
You can configure start options using a YAML-format config file when starting OriginHub server or worker. Here is a
complete example:
...
ssl_ca_certfile: /path/to/ca-bundle.pem
