...
OriginHub supports various environment variables for configuration.
...
## OriginHub Core Environment Variables
...
- **Server** - Applies to the OriginHub server.
- **Worker** - Applies to OriginHub workers.
...
| Variable                                           | Description                                                                                                                                                                                                                | Default   | Applies to |
| -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ---------- |
| `GPUSTACK_GATEWAY_PORT_CHECK_INTERVAL`             | The interval in seconds of OriginHub Server checking embedded gateway listening port                                                                                                                                        | `2`       | Server     |
| `GPUSTACK_GATEWAY_PORT_CHECK_RETRY_COUNT`          | The retry count of OriginHub Server checking embedded gateway listening port                                                                                                                                                | `300`     | Server     |
...
`GPUSTACK_GATEWAY_MIRROR_INGRESS_NAME`, and every ingress OriginHub generates for
...
| `GPUSTACK_RESOURCE_USAGE_TICK_SECONDS`             | Interval (seconds) for the instance-usage collector's periodic tick, which flushes accumulated runtime for long-running GPU/CPU instances that have had no lifecycle event since the last tick.                                                    | `300`        | Server     |
| `GPUSTACK_STORAGE_USAGE_TICK_SECONDS`              | Interval (seconds) for the persistent-volume (storage) usage collector's periodic tick.                                                                                                                                                            | `300`        | Server     |
| `GPUSTACK_METERED_USAGE_SEAL_GRACE_SECONDS`        | Grace window (seconds) after an hour-bucket elapses before it is sealed (finalized). A bucket is sealed once `now >= bucket_end + grace`, absorbing late events / clock skew; keep it larger than the tick interval. Sealed buckets are immutable. | `900`        | Server     |
| `GPUSTACK_METERED_USAGE_RETENTION_MONTHS`          | Retention window for `metered_usage` (the hourly resource-usage rollup). Sealed rows older than this are moved to `metered_usage_archive` by the leader-only archiver. Must stay well above the collector's settlement horizon (hours).            | `13`         | Server     |
| `GPUSTACK_METERED_USAGE_ARCHIVE_CRON`              | Cron expression (UTC) for the metered-usage archiver's recurring sweep. The archiver also runs once on server startup regardless of this schedule.                                                                                                 | `0 4 * * *`  | Server     |
| `GPUSTACK_METERED_USAGE_ARCHIVE_BATCH_SIZE`        | Per-batch row count for metered-usage archival moves. Smaller batches keep transactions short; larger batches reduce round-trip overhead.                                                                                                          | `5000`       | Server     |
...
!!! note
    ...
    These environment variables are **not** set when starting OriginHub. Instead, they should be configured in the **Advanced Options > Environment Variables** section when deploying a model. They are used to customize the model serving behavior.
...
| `GPUSTACK_SKIP_GPU_COUNT_CHECK`                           | Skips the validation that the manually selected GPU count must equal the parallelism world size (tp×pp×dp). Only takes effect when GPUs are manually selected; auto-scheduling is never affected. Use for manual single-node deployments where the parallelism parameters intentionally differ from the selected GPU count. | (empty) | Model          |
...
| `GPUSTACK_MODEL_INFERENCE_HEALTH_CHECK_ENABLED`           | Enable inference health check for this model. When enabled, OriginHub periodically sends minimal inference requests to verify the model is responding correctly.                                                                                                                                                             | `false` | Model          |
...
## OriginHub Runtime Environment Variables

These environment variables are used by OriginHub runtime. Commonly used to adjust the behavior of inference backends running in Docker/Kubernetes.
...
| `GPUSTACK_RUNTIME_DETECT_PCI_CLASS_PREFIXES`       | The PCI class prefixes to check during detection. When `GPUSTACK_RUNTIME_DETECT_NO_PCI_CHECK` is false, the detector will check if the PCI class of the device starts with any of the given prefixes.                                                                                                                                                                                                                                                                                                                                      | 02,03,0b,12                             |
...
| `GPUSTACK_RUNTIME_DEPLOY_DEFAULT_CONTAINER_REGISTRY_USERNAME`      | Username for the default container registry. If OriginHub runner images are hosted in a container registry that requires authenticated access, set this variable to the access username at the startup of every OriginHub worker (or the all-in-one OriginHub server). In practice, it is recommended to configure the access username in the actual container runtime instead. | (empty) |
| `GPUSTACK_RUNTIME_DEPLOY_DEFAULT_CONTAINER_REGISTRY_PASSWORD`      | Password for the default container registry. If OriginHub runner images are hosted in a container registry that requires authenticated access, set this variable to the access password at the startup of every OriginHub worker (or the all-in-one OriginHub server). In practice, it is recommended to configure the access password in the actual container runtime instead. | (empty) |
| `GPUSTACK_RUNTIME_DEPLOY_DEFAULT_CONTAINER_NAMESPACE`              | Namespace for default runner container images. If OriginHub runner images are hosted outside the `gpustack` namespace, e.g. `gpustack-ai`, set this variable to `gpustack-ai`. For a multi-level namespace, e.g. `awesome.com/amazing-group/amazing-subgroup/gpustack-ai`, declare the registry with the parent namespace via `--system-default-container-registry` (or the worker `system_default_container_registry` config key, or the `GPUSTACK_SYSTEM_DEFAULT_CONTAINER_REGISTRY` environment variable), e.g. `--system-default-container-registry=awesome.com/amazing-group/amazing-subgroup`. | `gpustack` |
