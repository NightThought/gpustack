...
async def build_local_auth_tables(
    ...
    * **soft-deleted keys.** ``ApiKey.delete()`` hard-deletes by default and no
      caller asks it for the soft path today, but the column exists, so the
      filter is here for the first one that does -- and it has to be, because
